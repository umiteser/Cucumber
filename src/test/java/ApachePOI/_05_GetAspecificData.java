...
public class _05_GetAspecificData {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Sheet sheet = workbook.getSheetAt(0); //sıfırdakini ver

        String aranan = "username";  //aradığım username
        String bulunan = "";
        for (int i = 0; i < sheet.getPhysicalNumberOfRows(); i++) {

            //bu satırın ilki hücresindeki bilginin küçük hali aranan bilgiye eşit mi demek
            if (sheet.getRow(i).getCell(0).toString().toLowerCase().equalsIgnoreCase(aranan))
            {
